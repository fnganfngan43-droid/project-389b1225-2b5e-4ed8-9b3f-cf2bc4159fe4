...
// Columns: التاريخ | رمز العملة | اسم المجموعة | اسم الحساب | مدين | دائن
...
// Columns: رقم السند | التاريخ | اسم المجموعة المدين | اسم الحساب المدين | رمز العملة المدين | المبلغ مدين | البيان المدين | رقم المرجع المدين | اسم المجموعة الدائن | اسم الحساب الدائن | رمز العملة الدائن | المبلغ الدائن | البيان الدائن | رقم المرجع الدائن
export function mapVoucherRow(row: any[]): {
  ...
  if (!row[3] && !row[9]) return null;
  
  return {
    voucherNumber: String(row[0] || ''),
    date: formatDate(row[1]),
    debitGroupName: String(row[2] || ''),
    debitAccountName: String(row[3] || ''),
    debitCurrency: String(row[4] || ''),
    debitAmount: parseFloat(row[5]) || 0,
    debitDescription: String(row[6] || ''),
    debitReference: String(row[7] || ''),
    creditGroupName: String(row[8] || ''),
    creditAccountName: String(row[9] || ''),
    creditCurrency: String(row[10] || ''),
    creditAmount: parseFloat(row[11]) || 0,
    creditDescription: String(row[12] || ''),
    creditReference: String(row[13] || ''),
  };
}

// Sales/Invoice import mapping
// Columns: التاريخ | رقم الفاتورة | النوع | اسم المجموعة | اسم الحساب | المبلغ | رمز العملة | رقم المرجع | البيان
...
// Columns: التاريخ | رقم الخصم | النوع | اسم المجموعة | اسم الحساب | المبلغ | رمز العملة | رقم المرجع | البيان
